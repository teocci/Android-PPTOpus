...
{
    private static final String TAG = LogHelper.makeLogTag(UtilHelper.class);
    ...
    {
        Matcher m1 = UtilHelper.VALID_IPV4_PATTERN.matcher(ipAddress);
        if (m1.matches()) {
            return true;
        }
        Matcher m2 = UtilHelper.VALID_IPV6_PATTERN.matcher(ipAddress);
        return m2.matches();
    }
    ...
    {
        Matcher m1 = UtilHelper.VALID_IPV6_PATTERN.matcher(ipAddress);
        return m1.matches();
    }
    ...
    public static List<String> getIPv4Addresses(String[] addresses)
    {
        if (addresses.length < 1) return null;
        List<String> ipV4Address = new ArrayList<>();

        boolean found = false;
        Matcher matcher;

        for (String ip : addresses) {
            matcher = VALID_IPV4_PATTERN.matcher(ip);
            if (matcher.matches()) {
                found = true;
                ipV4Address.add(ip);
            }
